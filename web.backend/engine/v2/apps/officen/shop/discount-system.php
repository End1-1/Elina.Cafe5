...
# Last Modified: 2025-10-06 23:59:32
...
class DiscountSystem extends Auth {
    ...
    public function GetInfo($params){
        ...
        if (empty($card)) {
            $this->exitError(Translator::t("Card with the specified code not found"));
        }
        if (!empty($params->need)) {
            if ($card["f_mode"] !== $params->need) {
                $this->exitError(Translator::t("Card type differs from the requested type"));
            }
        }
