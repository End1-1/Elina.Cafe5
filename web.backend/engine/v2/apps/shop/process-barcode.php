...
class ProcessBarCode extends Auth
{
    ...
    public function checkGiftCard()
    {
        ...
        $card = $this->select("select * from b_gift_card where f_code=?", "s", [$code])->fetch_assoc();
        if (empty($card)) {
            return false;
        }
        ...
        if (empty($card["f_datesaled"])) {
            ...
            $used = $this->select("select sum(cast(f_amount as float)) as f_amount from b_gift_card_history where f_card=?", "i", [$card["f_id"]])->fetch_assoc();
            if (empty($used)) {
                dieWithCode(Translator::t("Incorrect data for gift card") . "<br>" . $this->code);
            }
        }
    ...
    {
        $card = $this->select("SELECT * from b_cards_discount WHERE f_code=?", "s", [$this->code])->fetch_assoc();
        if (empty($card)) {
            return false;
        }
        if ($this->permissions->check(CP_T5_DISCOUNT)) {
            dieWithCode(Translator::t("You cannot use discount"));
        }
        $partner = $this->select("select * from c_partners where f_id=?", "i", [$card["f_client"]])->fetch_assoc();
        $history = $this->select("select coalesce(SUM(f_amount), 0) as f_amount FROM b_gift_card_history WHERE f_card=?", "i", [$card["f_id"]])->fetch_assoc();
        $this->result["card"] = $card;
    ...
    private function getGoods($echoresult = true)
    {
        ...
        $sql = <<<EOD
        SELECT g.f_id, g.f_name, gp.f_name as f_groupname, g.f_scancode,  g.f_lowlevel,
        ...
        if ($goods["f_service"] == 0) {
            $havechanges = $this->select("select f_have_changes from c_storages where f_id=? and f_have_changes=1", "i", [$this->params->store])->fetch_assoc();
            if (!empty($havechanges)) {
                $this->select("delete from a_store_sale where f_store=?", "i", [$this->params->store], true);
                $this->select("insert into a_store_sale (f_store, f_goods, f_qty, f_qtyreserve, f_qtyprogram) "
                    ...
                    . "from a_store where f_store=? group by 1, 2 having sum(f_qty*f_type)>0", "ii", [$this->params->store, $this->params->store], true);
                $this->select("update c_storages set f_have_changes=0 where f_id=?", "i", [$this->params->store], true);
            }
            ...
            if ($store["f_qty"] - $store["f_reserved"] - $store["f_draft"] - $goods["f_defaultqty"] < 0) {
                dieWithCode(Translator::t("Insufficient quantity") . " " .
                    ...
                    . "<br>" . Translator::t("Reserved") . " " . $store["f_reserved"]
                    . "<br>" . Translator::t("In draft") . " " . $store["f_draft"]);
            }
        }
        ...
        $this->insert("o_draft_sale_body", $v);
