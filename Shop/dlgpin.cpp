    ...
    if(ui->lePin->text().length() == 4 && ui->leUser->text().length() == 4) {
        fHttp->createHttpQueryLambda(
            ...
            {{"method", 1},
             ...
             {"forceconfig", C5ConnectionDialog::instance()->settings()}},
            [this](const QJsonObject &jdoc) {
                ...
                C5Database::fUrl = QString("%1://%2").arg(C5ConnectionDialog::instance()->connectionType() == C5ConnectionDialog::noneSecure
                                                              ? "http"
                                                              : "https",
