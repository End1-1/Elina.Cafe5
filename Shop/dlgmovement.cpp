      ...
      mSetupComplete(false)
{
    ...
    fStoreOut = mWorkStation.defaultStoreId();
...
bool DlgMovement::openDoc(const QString &doc)
{
    ...
    if (mWorkStation.defaultStoreId() != doc_header_store["f_storeout"].toInt()) {
        ui->btnSave->setEnabled(false);
    }
    ...
    ui->btnSaveAndAccept->setEnabled(fState == DOC_STATE_DRAFT && mWorkStation.defaultStoreId() == doc_header_store["f_storein"].toInt());
...
{
    fHttp->createHttpQuery("/engine/shop/movement.php",
                           QJsonObject{{"action", "search"}, {"store", mWorkStation.defaultStoreId()}, {"barcode", ui->leSearch->text()}},
