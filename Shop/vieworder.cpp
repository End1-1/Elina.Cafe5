...
void ViewOrder::on_btnSaveReturn_clicked()
{
    ...
    for(int j = 0; j < rows.count(); j++) {
        ...
        if(haveStore && ui->tbl->getInteger(i, 8) == 0) {
            ...
            if(!dw.writeAStoreDraft(adraftid, storeDocId, mWorkStation.defaultStoreId(), 1,
        ...
        g.store = mWorkStation.defaultStoreId();
    ...
    if(haveStore) {
        ...
        if(!dw.writeAHeaderStore(storeDocId, uid, uid, "", QDate::currentDate(), mWorkStation.defaultStoreId(), 0, 1, fCashUuid, 0,
                                 0, oheader._id())) {
            return returnFalse(dw.fErrorMsg, &db);
        }
    }
