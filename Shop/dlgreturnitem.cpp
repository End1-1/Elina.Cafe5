...
void DlgReturnItem::on_btnReturn_clicked()
{
    ...
    storedocUserNum = dw.storeDocNum(DOC_TYPE_STORE_INPUT, mWorkStation.defaultStoreId(), true, 0);
    ...
    for(int j = 0; j < ui->tblBody->rowCount(); j++) {
        ...
        if (!dw.writeAStoreDraft(adraftid,
                                 storeDocId,
                                 mWorkStation.defaultStoreId(),
                                 1,
                                 storeid,
                                 l->getDouble(),
                                 price,
                                 price * l->getDouble(),
                                 DOC_REASON_SALE_RETURN,
                                 adraftid,
                                 j + 1,
                                 "")) {
            C5Message::error(dw.fErrorMsg);
    ...
    if (!dw.writeAHeaderStore(storeDocId,
                              oheader.staff,
                              oheader.staff,
                              "",
                              QDate::currentDate(),
...
{
    fHttp->createHttpQuery("/shop/check-qty.php",
                           QJsonObject{{"store", mWorkStation.defaultStoreId()}, {"barcode", ui->leExchange->text()}},
