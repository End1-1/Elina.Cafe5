...
bool C5ShopOrder::write()
{
    ...
    //    if (needStoreDoc) {
    //        storedocUserNum = dw.storeDocNum(DOC_TYPE_STORE_OUTPUT, mWorkStation.defaultStoreId(), true, 0);
    ...
    //            return returnFalse(dw.fErrorMsg, db);
    //        }
    //        if (!dw.writeAHeaderStore(storeDocId, fOHeader.staff, fOHeader.staff, "", QDate(), 0, mWorkStation.defaultStoreId(), 1, "", 0, 0, fOHeader._id())) {
    //            return returnFalse(dw.fErrorMsg, db);
    //        }
    //    }
    //    for (int i = 0; i < fOGoods.count(); i++) {
    ...
    //        g.header = fOHeader._id();
    //        if (!g.isService) {
    //            if (!dw.writeAStoreDraft(g.storeRec, storeDocId, mWorkStation.defaultStoreId(), -1, g.goods, g.qty / g._qtybox,
